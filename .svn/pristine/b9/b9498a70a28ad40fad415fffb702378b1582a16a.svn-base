...
public class ProductColorAdapter extends BaseAdapter {
    ...
    // 1:未开放预约,2:立即预约，3：预约已过期，4：未开放购买，5：立即购买，6：已售罄，9：已预约
    ...
    private final String GER_HAD_BOOK = "ger_had_book"; // 已预约

    // private final String GER_PAY_ING = "ger_pay_ing";
    // private final String GER_PAY_ED = "ger_pay_ed"; //已预约
    ...
    public View getView(final int position, View view, ViewGroup parent) {
        ...
        if (view == null) {
            ...
            viewHolder.productContainerLL = (LinearLayout) view.findViewById(R.id.rl_product_container);
        ...
        Integer proClass = productBean.getProClass();
        ...
        viewHolder.nameTV.setText(proBrand + " " + proName + " " + proColor);
        viewHolder.netTypeTv.setText(proRom + " " + proNetype);
        ...
        // } else { //1:未开放预约,2:立即预约，3：预约已过期，4：未开放购买，5：立即购买，6：已售罄,9:已预约
        viewHolder.productOperatiorBtn.setVisibility(View.VISIBLE); 
        if ("1".equals(sellType)) {
            ...
            viewHolder.productOperatiorBtn.setEnabled(true);
        } else if ("3".equals(sellType)) {
            tempTypeString = GER_BOOK_AFTER;
            desc = mContext.getString(R.string.wait_next_book);
            viewHolder.productOperatiorBtn.setEnabled(false);
        } else if ("4".equals(sellType)) {// 未开放购买即禁止购买。此刻显示已售罄
            ...
            viewHolder.productOperatiorBtn.setEnabled(true);
            if (proClass != 1) {
                desc = mContext.getString(R.string.add_shopping_cart);
            }
        } else if ("6".equals(sellType)) {
            tempTypeString = GER_SELL_OUT;
            desc = mContext.getString(R.string.sell_out);
            viewHolder.productOperatiorBtn.setEnabled(false);
        } else if ("9".equals(sellType)) {// 所有预约之后的情景都显示为已预约，并不再支持下一步操作。
            tempTypeString = GER_HAD_BOOK;
            desc = mContext.getString(R.string.had_book);
            viewHolder.productOperatiorBtn.setEnabled(false);
        }else{
        	viewHolder.productOperatiorBtn.setVisibility(View.GONE);
        }
        /*
         * else if ("11".equals(sellType)) { tempTypeString = GER_PAY_ING; desc = mContext.getString(R.string.i_need_pay); }
		 * else if ("12".equals(sellType)) { tempTypeString = GER_PAY_ED; desc = mContext.getString(R.string.had_payed);
		 * viewHolder.productOperatiorBtn.setEnabled(false); }
		 */
        // }
        ...
        viewHolder.productOperatiorBtn.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if (mProductOperatorListener != null) {
                     ...
                     * 所有预约之后的情景都显示为已预约，并不再支持下一步操作。需要下单或者付款的，去我的预约界面。 else if (GER_PAY_ING.equals(mType)) {
					 * mProductOperatorListener.pay(productBean.getId() + ""); }
					 */
                } else {
