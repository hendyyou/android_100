...
public class ShoppingFragment extends BaseFragment<ShoppingCartBean> {
    ...
    private RelativeLayout mShoppingcartItemFLContainter;
                ...
                if (outBean != null) {
                    if (outBean.getShoppingCartVOList() != null && outBean.getShoppingCartVOList().size() > 0) {
                        ...
                        if (mProductItems != null && mProductItems.size() > 0) {
                            mShoppingCartEmptyLL.setVisibility(View.GONE);
                            mShoppingcartItemFLContainter.setVisibility(View.VISIBLE);
                            String totalPrice = getString(R.string.order_list_rmb)+Utils.Money.MoneyToYuan(mShoppingCartBean.getProductPriceAmt());
                    ...
                    Utils.Preference.setIntPref(getParent(), Constants.DataFile.DefaultKey.SHOPPING_COUNT, outBean.getProductCount());
                    getParent().updateShoppingCount(outBean.getProductCount());
                } else {
                    Utils.Preference.setIntPref(getParent(), Constants.DataFile.DefaultKey.SHOPPING_COUNT, 0);
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        mShoppingcartItemFLContainter = (RelativeLayout) view.findViewById(R.id.fl_shopiing_cart_item_container);
